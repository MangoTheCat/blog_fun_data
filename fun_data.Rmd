---
title: 'Fun data: open data with a sense of humour'
output: 
  html_document:
    keep_md: true
---
...
ingredients <- foodFacts %>%
  # ideally, the unnest_tokens function is what we want but it required too much time
  #tidytext::unnest_tokens(ingredient, ingredients_text,
  #                        token = stringr::str_split, pattern=",|\\(|\\)|\\[|\\]") %>%
  # so instead we chose a different approach involving mutate and unlist
  # mutate will give us a list-column
  mutate(ingredients = stringr::str_split(ingredients_text, pattern=",|\\(|\\)|\\[|\\]")) %>%
  select(ingredients) %>%
